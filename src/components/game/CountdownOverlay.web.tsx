// === Cinematic Countdown Overlay — Web (no Skia) ===
...
function CountdownNumber({ value }: { value: number }) {
  ...
  return <Animated.Text style={[styles.number, animatedStyle]}>{value}</Animated.Text>;
}
...
function GoText() {
  ...
  return <Animated.Text style={[styles.goText, animatedStyle]}>GO!</Animated.Text>;
}
...
export function CountdownOverlay({ onComplete }: CountdownOverlayProps) {
  ...
  useEffect(() => {
    ...
    const timer1 = setTimeout(() => {
      setCount(2); setRingPulse(p => p + 1);
      void Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Heavy);
      void playSound('tick');
    }, 1000);

    const timer2 = setTimeout(() => {
      setCount(1); setRingPulse(p => p + 1);
      void Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Heavy);
      void playSound('tick');
    }, 2000);

    const timer3 = setTimeout(() => {
      setShowGo(true); setRingPulse(p => p + 1);
  ...
  return (
    <View style={styles.container}>
      ...
      {!showGo ? <CountdownNumber value={count} key={count} /> : <GoText />}
      {!showGo && (
        <Animated.Text entering={FadeIn.duration(300)} style={styles.label}>
          GET READY
        </Animated.Text>
      )}
    </View>
...
const styles = StyleSheet.create({
  ...
  number: { fontSize: 140, fontWeight: '900', color: COLORS.accent, zIndex: 10 },
  goText: { fontSize: 100, fontWeight: '900', color: COLORS.alive, letterSpacing: 8, zIndex: 10 },
  label: { fontSize: 16, fontWeight: '700', color: COLORS.textDim, letterSpacing: 6, marginTop: 8, zIndex: 10 },
  ringContainer: { position: 'absolute', width: 120, height: 120, alignItems: 'center', justifyContent: 'center' },
